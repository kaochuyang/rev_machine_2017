...
/*void *thread(void*)
{while(1)
    {if(smem.o_Junbo_light.REV_light_port.Rs232Read()>0)
//    smem.o_Junbo_light.junbo_light_receive("/home/cct/Desktop/Untitled4/tc2016-for_rev_light_2017/junbo_light_record.txt");
    }
}*/
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        //  smem.o_Junbo_light.initial_junbo_control("/home/cct/Desktop/Untitled4/tc2016-for_rev_light_2017/junbo_light_record.txt","/dev/ttyS1");       //2017 05 24 junbolight contorol

        smem.o_Junbo_light.initial_junbo_control("/cct/Data/junborecord/junborecord.txt","/dev/ttyS0");
        //  pthread_t id;
        /*    int ret3=0;
            ret3=pthread_create(&id,NULL,&thread,NULL);
            printf("\n ret33=%d\n",ret3);
            if(ret3!=0)
            {
...
////-----------------------------------------------------------------------------------------------------//
        ...
        if( 1 == ucTmp )
        {
            if (smem.revLaneManualPort.SetConnDevice(DEVICEREVMANUALPAD))
                if ((tempmax=smem.revLaneManualPort.OpenRs232Port("/dev/ttyS10",2400, true))>0)
                {
                    if (tempmax>maxport)
                    {
                        maxport = tempmax;
                    }
                    printf("open revLaneManualPort-/dev/ttyS10 (fdValue:%d) Success!!\n",tempmax);
                }
...
//OT990618
            ...
            if(smem.o_Junbo_light.REV_light_port.GetPortAlreadyOpen())FD_SET(smem.o_Junbo_light.REV_light_port.Getfd(),&readfs);
            /*
...
//OT20111116
                            if(smem.vGetCommEnable() == true)
                            {
                                ...
                                parseAABB.EchoToGUI(&smem.revLaneManualPort.maxMessageIndex,smem.revLaneManualPort.messageIn,"/dev/ttyS10");
...
//OTSS--
                ...
                if(smem.o_Junbo_light.REV_light_port.GetPortAlreadyOpen())
                {
                    if(FD_ISSET(smem.o_Junbo_light.REV_light_port.Getfd(),&readfs))
                    {
                        readSelectLength=smem.o_Junbo_light.REV_light_port.Rs232Read();
                        if(readSelectLength>0)
                        {
                            for(int i =0; i<readSelectLength; i++)
                                printf("%x ",smem.o_Junbo_light.REV_light_port.block[i]);

                            printf("\n\n");

                            smem.o_Junbo_light.ParseBlock(readSelectLength,smem.o_Junbo_light.REV_light_port.block,smem.o_Junbo_light.REV_light_port.messageIn,
                                                          &smem.o_Junbo_light.REV_light_port.lastPacketIndex);
